  ...
  switch (id) {
    ...
    case ACCEL_ID: {
      if (ACCEL_DATA_LEN == len) {
        ...
        // ROS_INFO("accel x: %.3f, accel y: %.3f, accel z: %.3f", g_output_info.accel_x, g_output_info.accel_y,
    ...
    case LOCATION_ID: {
      if (LOCATION_DATA_LEN == len) {
        ...
        // ROS_INFO("location: %.3f, %.3f, %.3f", g_output_info.longtidue, g_output_info.latitude,
        // g_output_info.altidue);
      } else {
        ret = (unsigned char)0x00;
      }
    } break;
    ...
    case GNSS_MASTER_ID: {
      ...
      if (ret) {
        ...
        std::memcpy(&g_output_info.gnss.utc_time, buf, 9);
        buf += 9;
